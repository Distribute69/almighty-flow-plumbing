          ...
          {services.map((service, i) => (
            <FadeIn key={service.title} delay={i * 0.06}>
              <article className="glass-card section-badge group relative flex h-full min-h-72 flex-col overflow-hidden p-7 transition-all duration-300">
                ...
                <div className="absolute right-5 top-5 font-heading text-5xl font-black text-white/[0.04]">
                  0{i + 1}
                </div>
                <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl border border-accent/40 bg-accent/15 text-accent-bright shadow-glow transition-all group-hover:scale-105 group-hover:border-gold/50 group-hover:text-gold-soft">
